...
│   ├── views/
│   │   ├── OrgBrowser.vue        # P0: 已完成
│   │   ├── OrgAdjustment.vue     # P0: 已完成
│   │   ├── Reconciliation.vue    # P1: 已完成
│   │   └── EntityMaster.vue      # P1: 已完成
│   ├── components/
│   │   ├── OrgTree.vue           # 已完成
│   │   ├── DiffViewer.vue        # 已完成
│   │   └── ValidationResult.vue  # 已完成
│   ├── mock/
│   │   ├── orgData.ts            # 已完成
│   │   └── validationRules.ts    # 已完成
│   ├── types/
...
1. OrgAdjustment.vue（组织调整）
2. Reconciliation.vue（监控对账）
3. EntityMaster.vue（实体主数据）

Reconciliation.vue 功能总结
✅ 数据健康分 - 仪表盘展示整体健康评分
✅ 统计卡片 - 实体总数、严重问题、警告、提示
✅ 健康检查按钮 - 手动触发校验引擎
✅ 校验结果展示 - 使用 ValidationResult 组件
✅ 差异分类汇总 - 数据源差异、树结构健康、跨树差异
✅ 差异明细 - 点击分类卡片查看详细问题列表
✅ 操作功能 - 立即同步、转工单、忽略、去修复
✅ 手动触发对账 - 选择日期和范围进行对账

EntityMaster.vue 功能总结
✅ 实体列表管理 - 搜索、筛选、分页展示
✅ 新增临时实体 - 带校验，检测游离实体警告
✅ 编辑实体 - 修改实体信息
✅ 删除临时实体 - 仅允许删除临时录入的实体
✅ 实体详情 - 展示完整信息和在各树中的位置
✅ 运行实体校验 - 检测游离实体、单树独有等问题
✅ 在树中查看 - 跳转到组织浏览页面并高亮节点
✅ 校验结果展示 - 使用 ValidationResult 组件
